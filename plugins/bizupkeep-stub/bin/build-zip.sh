#!/usr/bin/env bash
#
# Build a WordPress-installable release ZIP for the BizUpKeep Stub
...
#   version defaults to the "Version:" header in bizupkeep-stub.php.

set -euo pipefail

cd "$(dirname "$0")/.."

VERSION="${1:-$(grep -m1 '^ \* Version:' bizupkeep-stub.php | sed -E 's/.*Version:[[:space:]]*//')}"
...
echo "Building bizupkeep-stub-${VERSION}.zip ..."

rm -rf build
mkdir -p build/bizupkeep-stub
...
if command -v rsync >/dev/null 2>&1; then
    rsync -a "${RSYNC_EXCLUDES[@]}" ./ build/bizupkeep-stub/
else
    shopt -s dotglob
    for entry in ./*; do
        name="$(basename "$entry")"
        ...
        cp -r "$entry" "build/bizupkeep-stub/$name"
    done
    shopt -u dotglob

    for item in "${EXCLUDES[@]}"; do
        rm -rf "build/bizupkeep-stub/${item}"
    done
fi

(
    cd build/bizupkeep-stub
...
(
    cd build
    rm -f "bizupkeep-stub-${VERSION}.zip"

    if command -v zip >/dev/null 2>&1; then
        zip -r -q "bizupkeep-stub-${VERSION}.zip" bizupkeep-stub
...
' "bizupkeep-stub-${VERSION}.zip" bizupkeep-stub
    else
...
echo "Built build/bizupkeep-stub-${VERSION}.zip"
