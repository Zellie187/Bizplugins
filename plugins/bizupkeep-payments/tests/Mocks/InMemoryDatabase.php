...
namespace BizHub\Payments\Tests\Mocks;
...
/**
 * In-memory DatabaseInterface implementation for tests. Copied from
 * bizupkeep-bookkeeping's tests/Mocks/InMemoryDatabase.php (itself
 * mirroring bizupkeep-workflow's and BizHub's own copies), so
 * repositories in this plugin are exercised against the same semantics
 * as every other repository in this codebase.
 *
 * Not a full SQL engine: findAll() only supports equality criteria and
 * single-column ordering, and query()/execute() are no-ops - sufficient
 * for every repository in this plugin, which never needs raw SQL (the
 * atomic status transition is achieved via update()'s exact-match
 * criteria instead - see PaymentAttemptRepositoryInterface's docblock).
 */
