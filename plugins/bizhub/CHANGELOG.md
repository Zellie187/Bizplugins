...
- `Documents\Entities\DocumentCategory` gained a `SIGNED_POA` case ("Signed Power of Attorney"). Additive - existing categories and their `label()` output are unaffected.
- Driven by `bizupkeep-workflow`/`astra-child`'s new Power of Attorney generation feature: Company Registration and Company Amendment applications now require a signed POA upload (generated by the theme, printed, signed by all directors, and uploaded back) instead of a Proof of Address, matching the workflow spec's actual document requirement.
- 106 passing PHPUnit tests (unchanged), PHPStan level 6 clean.
